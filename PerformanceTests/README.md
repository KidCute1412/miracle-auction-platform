...
The latest local HA bid-path diagnostic (`bid-path-20260821085816-35e03e`) used `bid-priority`, five independent 75-second runs and 100 VUs: median throughput `333.83 req/s`, accepted bids `323.90/s`, p95 `487.10 ms`, p99 `606.95 ms`, and throughput CV `5.79%`. Core invariants passed. Transport/server crash errors were `0%`; one request (`0.0038%`) received the intentional fail-closed `BID_DURABILITY_UNCONFIRMED` response because replica acknowledgement exceeded the 100 ms bound. This is diagnostic evidence with downstream consumers intentionally skipped and is not an official end-to-end or production-capacity claim. The paired report and configuration are preserved under `artifacts/runs/bid-path-20260821085816-35e03e/`.
