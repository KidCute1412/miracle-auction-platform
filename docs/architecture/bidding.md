...
A single `auction-worker` owns the global Stream consumer group. It reads up to 200 entries, groups them by product, preserves sequence within each product lane, and processes independent lanes with bounded concurrency (default 8). Each healthy lane prefetches its product/processed sequences once, validates a rolling fence, bulk-inserts transitions, history, receipts and outbox, then updates the product from the final event in one transaction. After commit it acknowledges and deletes that exact Stream entry; startup compaction trims only below the oldest pending entry. A failed lane falls back to isolated entry handling so retry/DLQ attribution remains precise. `XAUTOCLAIM` runs on a cadence. Shutdown waits for all active lanes.
