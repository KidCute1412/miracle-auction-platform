...
api.lok1412.site      -> Hetzner EU VPS: Caddy, API, 3 workers, Redis
Supabase               -> PostgreSQL
Aiven Free             -> Kafka
```
...
1. Vercel frontend project → **Settings** → **Environment Variables** → add Production value:
   ...
   ```text
   VITE_API_URL=https://api.lok1412.site
   ```
...
3. Confirm in a private browser:
   ...
   ```text
   https://api.lok1412.site/health  -> 200
   https://api.lok1412.site/ready   -> 200
