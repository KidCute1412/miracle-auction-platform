...
export default function VisitorAnalyticsPage() {
  ...
  return <section className="space-y-5" aria-labelledby="visitor-title">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Metric icon={Clock3} label="Average Duration" value={summary ? duration(summary.averageDurationSeconds) : "—"} />
    </div>
    <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
      <form className="grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(260px,1fr)_repeat(5,minmax(140px,auto))]" onSubmit={(event) => { event.preventDefault(); update("search", draftSearch.trim() || undefined); }}>
        <label className="relative"><span className="sr-only">Search sessions</span><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><input value={draftSearch} onChange={(event) => setDraftSearch(event.target.value)} placeholder="IP, visitor ID, username, email, URL..." className="h-9 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm focus-visible:ring-2 focus-visible:ring-ring" /></label>
        ...
        <button className="h-9 rounded-md bg-primary px-5 text-sm font-semibold text-primary-foreground">Search</button>
      </form>
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm">
