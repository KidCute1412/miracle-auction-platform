...
export function assertBidAmount(auction: AuctionState, maximum: bigint): void {
  const minimum = auction.currentPrice + auction.stepPrice;
  const followsStep = auction.stepPrice === 0n || (maximum - auction.currentPrice) % auction.stepPrice === 0n;
  if (maximum < minimum || !followsStep) throw new BidDomainError("Invalid bid price");
}

export function assertBuyNowAvailable(auction: AuctionState, userId: number, amount: bigint): void {
