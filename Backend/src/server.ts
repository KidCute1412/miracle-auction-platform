...
httpServer.listen(port, async () => {
  ...
  outboxTimer = setInterval(() => void dispatchBidOutbox().catch(console.error), 1000);
  outboxTimer.unref();
});
...
async function gracefulShutdown(signal: string) {
  ...
  await Promise.allSettled([stopAdminSocket(), prisma.$disconnect(), closeRedisConnection(), closeKafkaConnection()]);
  process.exit(0);
}
