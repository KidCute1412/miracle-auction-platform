...
async function markRetry(rows: RelayRow[], error: unknown, maxBackoffMs: number): Promise<void> {
  ...
  await Promise.all(rows.map((row) => prisma.auction_outbox.updateMany({
...
export async function relayOutboxBatch(options?: {
  ...
  if (valid.length > 0) {
    try {
      await publishEventBatchesStrict([...groups].map(([topic, rows]) => ({
        topic,
        messages: rows.map((row) => ({
          key: row.aggregate_id,
          value: JSON.stringify(createEventEnvelope({
...
async function writeHeartbeat(): Promise<void> {
  ...
  await redisClient.set("outbox:relay:heartbeat", new Date().toISOString(), "EX", ttl).catch((error) => {
    console.warn("[OUTBOX_RELAY] Redis heartbeat unavailable; relay continues", {
  ...
  relayLoop = (async () => {
    while (running) {
      try {
        ...
        console.error("[OUTBOX_RELAY] Poll failed", {
          message: error instanceof Error ? error.message : "unknown",
        });
