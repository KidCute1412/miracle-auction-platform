...
describe("profile use cases", () => {
  it("updates a profile without an avatar", async () => {
    ...
    await expect(editUserProfile({ user_id: 1 })).resolves.toEqual({ user_id: 1 });
    expect(upload).not.toHaveBeenCalled();
  });

  it("uploads and removes a temporary avatar", async () => {
const data: import("../../../src/modules/profiles/infrastructure/profile.repository.ts").EditProfileInput = { user_id: 1 };
