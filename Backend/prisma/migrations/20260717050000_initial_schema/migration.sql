...
create table public.dashboard_stats (
  ...
  constraint dashboard_stats_pkey primary key (key)
) TABLESPACE pg_default;
...
create table public.bidding_ban_user (
  ...
  constraint bidding_ban_user_product_user_unique unique (product_id, user_id),
...
create table public.orders (
  ...
  constraint orders_one_per_product_unique unique (product_id),
...
create index bidding_history_product_created_idx on public.bidding_history (product_id, created_at);

create table public.bid_idempotency (
  id bigint generated by default as identity not null,
  ...
  created_at timestamp with time zone not null default now(),
  constraint bid_idempotency_pkey primary key (id),
  constraint bid_idempotency_operation_user_key_unique unique (operation, user_id, idempotency_key),
  constraint bid_idempotency_user_id_fkey foreign key (user_id) references users (user_id)
) TABLESPACE pg_default;

create table public.auction_outbox (
  id bigint generated by default as identity not null,
  ...
  created_at timestamp with time zone not null default now(),
  constraint auction_outbox_pkey primary key (id)
) TABLESPACE pg_default;

create index auction_outbox_pending_idx on public.auction_outbox (delivered_at, available_at, id);
